...
(function() {
  ...
  Humane_List = (function() {
    ...
    Humane_List.prototype.pop = function(pos) {
      var arr;
      arr = pos.toString() === 'front' ? this.data.core.shift() : this.data.core.pop();
      if (!arr) {
        return arr;
      }
      return arr.val;
    ...
    Humane_List.prototype.push = function() {
      ...
      args = arguments;
      pos = args[0];
      num_pos = typeof pos === "number" ? pos : pos === 'front' ? 1 : (last = this.positions().pop(), (last && (last + 1)) || this.data.core.length + 1);
      switch (args.length) {
        case 2:
      ...
      this.data.core.unshift(e);
      this.data.core = this.data.core.sort(function(a, b) {
        return a.pos > b.pos;
      });
      if (this.data.core.length > 1) {
        _ref = this.data.core;
        for (i = _i = 0, _len = _ref.length; _i < _len; i = ++_i) {
          last_v = _ref[i];
          v = this.data.core[i + 1];
          if (v) {
            if (last_v.pos >= v.pos) {
              v.pos += 1;
            }
    ...
    Humane_List.prototype.front = function() {
      return this.data.core[0] && this.data.core[0].val;
    };

    Humane_List.prototype.end = function() {
      var row;
      row = this.data.core[this.data.core.length - 1];
      return row && row.val;
    };
    ...
    Humane_List.prototype.at_position = function(n) {
      ...
      return this.data.core[comp_pos] && this.data.core[comp_pos].val;
    };
    ...
    Humane_List.prototype.delete_at = function(target_k) {
      ...
      row = this.data.core[pos];
      if (!row) {
        return row;
      }
      old = this.at_position(pos);
      this.data.core = (function() {
        var _i, _len, _ref, _results;
        _ref = this.data.core;
        _results = [];
        for (k = _i = 0, _len = _ref.length; _i < _len; k = ++_i) {
